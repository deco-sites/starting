...
export default function PopUp({ buttonLabel, items, theme = "dark" }: Omit<Props, "@Page"> & { theme?: "dark" | "ligth" }) {
    ...
    return (
        <>
            < button className={`cursor-pointer  text-sm ${THEME[theme]}`} onClick={handlerClick}> {buttonLabel}</button >
            <dialog ref={modal} class="modal rounded-3xl p-6 border-slate-500 border bg-[#000]">
