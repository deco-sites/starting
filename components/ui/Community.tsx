  ...
  return (
    <section class="flex flex-col items-center justify-center lg:(flex-row-reverse gap-[75px]) bg-white py-[63px] px-6 md:px-[2rem] max-w-screen-2xl mx-auto">
      <div class="relative lg:w-[50%]">
        <Image fetchPriority="high" preload={true} loading={lazyload ? 'lazy' : 'eager'} class="relative w-full" width={581} height={581} src={blurBackground}/>
        <Image fetchPriority="high" preload={true} loading={lazyload ? 'lazy' : 'eager'} class="absolute w-full top-0" width={537} height={497} src={peopleImage}/>
      </div>
      <div class="flex flex-col lg:w-[50%] gap-[24px] mt-[40px]">
        ...
        <p class="text-[32px] lg:order-3 text-[#1f261f99] leading-[38px]">{secondText}</p>
        <div class="flex flex-col lg:(flex-row order-2) w-full gap-[16px]">
          {
            labelFirstButton &&
            hrefFirstButton &&
            <button class="w-full group flex lg:max-w-[300px] lg:justify-left justify-center items-center gap-[10px] rounded p-[14px] border-1 border-[#0A2121]">
              <a class="text-[16px] font-medium text-[#0A2121]" href={hrefFirstButton}>{labelFirstButton}</a>
              <Icon class="hidden transition lg:group-hover:block" id="GreenArrow" width={15} height={15} strokeWidth={"1"} />
            </button>
          }
          <button class={`flex group lg:max-w-[300px] lg:justify-left justify-center items-center gap-[10px] w-full rounded p-[14px] border-1 border-[#0A2121] bg-[#0A2121]`}>
            ...
            <a class="text-[16px] font-medium text-[#fff]" href={hrefSecondButton}>{labelSecondButton}</a>
