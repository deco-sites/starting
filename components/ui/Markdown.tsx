...
export function linkAndImage(block: string): string {
    ...
    if (matches) {
        for (const match of matches) {
            ...
            if (isImage) {
                const urlDeco = `/live/invoke/deco-sites/std/loaders/x/image.ts?src=${encodeURIComponent(href)}&fit=contain&width=1080 720w, /live/invoke/deco-sites/std/loaders/x/image.ts?src=${encodeURIComponent(href)}&fit=contain&width=2160 1440w`;
                block = block.replace("!" + match, `<img src="${href}" alt="${label}" decoding="async" loading="lazy" srcset=${urlDeco}">`);
            }
            else {
...
export default function Markdown({ post }: Props) {
    ...
    return (<div class="markdown-body prose max-w-none text-[#F9FAFB] tracking-[-0.16px]" dangerouslySetInnerHTML={{ __html: body }}/>);
}
