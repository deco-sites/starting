 ...
 */
const gitHubRawLoader: LoaderFunction<Props, string> = async (_req, ctx, { repo, branch, path }) => {
    const pathFromParams = ctx.params.path !== ":path" && ctx.params.path;
    const resultPath = pathFromParams || path;
    const res = await fetch(`https://raw.githubusercontent.com/${repo}/${branch}/${resultPath}`).then((res) => res.text());
    return {
        data: res,
    };
};
