 ...
 */
const gitHubRawLoader: LoaderFunction<Props, PostList> = async (_req, _ctx, { repo, branch, path }) => {
    const resultPath = path;
    const res = await fetch(`https://raw.githubusercontent.com/${repo}/${branch}/${resultPath}`).then((res) => res.json() as Promise<PostList>);
    return {
        data: res,
    };
};
