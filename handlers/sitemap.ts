...
import { type Resolvable, isResolvable } from "@deco/deco";
const isPage = (handler: Resolvable<Handler>) => isResolvable(handler) && handler.__resolveType.endsWith("handlers/fresh.ts");
const isAbsolute = (href: string) => !href.includes(":") && !href.includes("*") && !href.startsWith("/_live");
...
const siteMapFromRoutes = (publicUrl: string, routes: Route[], excludePaths?: string[]): string => {
    const urls: string[] = [];
    const excludePathsSet = new Set(excludePaths);
    for (const route of routes) {
        if (!excludePathsSet.has(route.pathTemplate) &&
...
</urlset>`, { headers: { "content-type": "text/xml", status: "200" } });
    };
}
