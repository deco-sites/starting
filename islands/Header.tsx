...
export default function Header(props: Props) {
    ...
    if(IS_BROWSER){
        ...
        if (urlPage.includes('dev')) {
            props.pageInitial = urlPage.includes('en') ? 'for developers' : 'para desenvolvedores'
...
return (
    ...
    <div class="md:hidden w-[100vw] h-[4px] fixed top-[60px] transition ease-in-out duration-300 left-0" style="background-image: linear-gradient(270deg, rgba(0,0,0,0) 0%, rgba(0,255,128,1) 100%);"></div>
        <ul class="h-full flex items-center">
            <li class="h-full">
                <a class="flex items-center h-full px-3 pb-[6px]" href={currentLanguageLink}>
        ...
        <li class="grid items-center">
            ...
            <svg
                ...
                class={open ? 'hidden' : ''}
            >
                <rect x="19" y="2" width="17" height="4" rx="2" fill="#06E474" />
                <rect x="11" y="14" width="25" height="4" rx="2" fill="#06E474" />
                <rect x="3" y="26" width="33" height="4" rx="2" fill="#06E474" />
            </svg>
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="#06E474" width="35" height="32" class={open ? '' : 'hidden'}>
        ...
        <div class={open ? "flex flex-col justify-between w-full h-[94vh] gap-[40px] absolute bg-[#0A2121] left-0 top-[63px] z-50 py-[24px] px-3 md:hidden" : "hidden"}>
            ...
            {props.menuLinks.map((link) => {
                ...
                <li class="h-[50px] grid items-center">
                    <a href={link.href} target={link.targetBlank ? '_blank' : '_self'} class="block px-[24px]  font-normal text-[16px] leading-[19.36px] text-white-79">{link.label}</a>
                </li>
                )
            })}
            </ul>
            ...
            <li class="h-[50px] grid items-center">
                <a href={props.linkedinUrl} target="_blank" class="block px-3  font-normal text-[16px] leading-[19.36px] text-[#2FD180]">
            ...
            <li class="h-[50px] grid items-center">
                <a href={props.gitUrl} target="_blank" class="block px-3  font-normal text-[16px] leading-[19.36px] text-[#2FD180]">
            ...
            <li class="h-[50px] grid items-center">
                <a href={props.discordUrl} target="_blank" class="block px-3  font-normal text-[16px] leading-[19.36px] text-[#2FD180]">
        ...
        <ul class="hidden lg:flex lg:flex-row lg:gap-4 px-3">
            ...
            <li>
            <a href={props.sign.url} class="block md:hover:(bg-[#0A2121] border-1 text-[#fff]) border-[#02F67C] border-1  bg-[#02F67C] font-normal text-[16px] text-[#0A2121] px-3 md:py-1 rounded-full md:transition md:ease-in-out md:duration-300">
            {props.sign.label}
            </a>
        </li>
        </ul>
