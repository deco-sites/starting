...
export default function Header(props: Props) {
    ...
    if(IS_BROWSER){
        ...
        if (urlPage.includes('dev')) {
            props.pageInitial = 'for developers';
...
return (
    ...
    <div class="md:hidden w-[100vw] h-[4px] fixed top-[60px] transition ease-in-out duration-300 left-0" style="background-image: linear-gradient(270deg, rgba(0,0,0,0) 0%, rgba(0,255,128,1) 100%);"></div>
        ...
        <div class={open ? "flex flex-col justify-between w-full h-[94vh] gap-[40px] absolute bg-[#0A2121] left-0 top-[63px] z-50 py-[24px] px-3 md:hidden" : "hidden"}>
            ...
            {props.menuLinks.map((link) => {
                ...
                <li class="h-[50px] grid items-center">
                    <a href={link.href} target={link.targetBlank ? '_blank' : '_self'} class="block px-[24px] font-inter font-normal text-[16px] leading-[19.36px] text-white-79">{link.label}</a>
                </li>
                )
            })}
            </ul>
            ...
            <li class="h-[50px] grid items-center">
                <a href={urlPortuguese.value} class="block px-3 font-inter font-normal text-[16px] leading-[19.36px] text-[#2FD180]">{props.pt.label}</a>
            </li>
            <li class="h-[50px] grid items-center">
                <a href={urlEnglish.value} class="block px-3 font-inter font-normal text-[16px] leading-[19.36px] text-[#2FD180]">{props.eng.label}</a>
            </li>
            <li class="h-[50px] grid items-center">
        ...
        <li class="group cursor-pointer md:relative">
            ...
            <span class="font-inter font-normal text-[16px] text-[#06E474] group-hover:text-[#fff]">{props.idiom}</span>
            <svg width="9" height="5" viewBox="0 0 9 5" fill="none" xmlns="http://www.w3.org/2000/svg">
        ...
        <li>
            <a href={props.login.url} class="block md:hover:(border-[#2FD180] border-1 text-[#fff]) border-[transparent] border-1 font-inter font-normal text-[16px] text-[#06E474] px-3 md:py-1 rounded-full md:transition md:ease-in-out md:duration-300">
            {props.login.label}
            </a>
        </li>
        </ul>
