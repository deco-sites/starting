import { getBlogPath, PostList, SupportedLocales, } from "site/components/utils/Blog.ts";
...
export default function BlogPostHeader(props: Props) {
    ...
    const post = postsList.posts.filter((x) => getBlogPath(x.path, locale) === url)[0];
    if (!post) {
        return <div>Post not found.</div>;
    }
    return (<section class="max-w-2xl w-full px-5 md:px-0 mx-auto pt-16">
      <div class="mb-8 sm:mb-11 mt-16">
        <a class="group flex items-center cursor-pointer text-[#F9FAFB]" href={`/${locale}/blog`}>
      ...
      <style dangerouslySetInnerHTML={{ __html: `body{background-color: black;}` }}>
      </style>
    </section>);
}
