...
function TemplatesGrid(props: Props) {
  ...
  const [mostrarCardsOutrasCategorias, setMostrarCardsOutrasCategorias] =
    useState(false);
  ...
  const itensFiltrados = mostrarCardsOutrasCategorias
    ? cards.filter((item) =>
      item.category && !indexCategories.includes(item.category)
    )
    : (categoriaSelecionada === "Todos"
      ? cards
  ...
  return (
    <div className="flex flex-col items-center mx-8 pb-10 md:mx-auto md:px-14 md:pb-16 lg:px-16 gap-y-10 lg:pb-20 lg:max-w-[1440px] xl:px-16">
      <div
        id="index"
        className="flex flex-start gap-x-12 h-[36px] w-full md:px-4"
      >
        <select
            ...
            <div
              ...
              className={`cursor-pointer ${
                (categoriaSelecionada === category &&
                    !mostrarCardsOutrasCategorias) ||
                  (mostrarCardsOutrasCategorias && category === "Outros")
